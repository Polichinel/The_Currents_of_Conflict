...
def get_Xy_tt(local = False, binary_y = True):
    ...
    if binary_y == True:
        ...
        y_train = np.log(df_merged[df_merged['train'] == 1]['ged_best_sb']+1) # try with log you tool
        y_test = np.log(df_merged[df_merged['train'] == 0]['ged_best_sb']+1) # try with log you tool.. and its val, not test
